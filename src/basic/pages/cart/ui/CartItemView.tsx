...
export function CartItemView({ item, cart, products, setCart, handleNotificationAdd }: CartItemViewProps) {
  ...
  function handleProductRemoveFromCart(productId: string) {
    setCart(removeItemFromCart(cart, productId));
  }

  function handleProductQuantityUpdate(productId: string, newQuantity: number) {
    if (newQuantity <= 0) {
  ...
  return (
    <div className="border-b pb-3 last:border-b-0">
      <div className="flex justify-between items-start mb-2">
        ...
        <button className="text-gray-400 hover:text-red-500 ml-2" onClick={() => handleProductRemoveFromCart(item.product.id)}>
          <IconRemoveFromCart />
        </button>
      </div>
