...
import App from "../App";
import "../../setupTests";
import { clearNotifications } from "../shared/libs/notificationStore";
...
describe("쇼핑몰 앱 통합 테스트", () => {
  ...
  describe("관리자 기능", () => {
    ...
    test("상품 재고 입력 시 9999 초과 검증이 작동한다", async () => {
      // 상품 수정 버튼 클릭
      const editButton = screen.getAllByText("수정")[0];
      fireEvent.click(editButton);

      // 재고 입력 필드에 9999 초과값 입력
      const stockInputs = screen.getAllByPlaceholderText("숫자만 입력");
      const stockInput = stockInputs[1]; // 재고 입력 필드는 두 번째

      // 5454355 입력
      fireEvent.change(stockInput, { target: { value: "5454355" } });
      fireEvent.blur(stockInput);

      // 에러 토스트 메시지 확인
      await waitFor(() => {
        expect(
          screen.getByText("재고는 9999개를 초과할 수 없습니다")
        ).toBeInTheDocument();
      });

      // 값이 9999로 수정되었는지 확인
      expect(stockInput.value).toBe("9999");
    });

    test("상품 가격 입력 시 빈 값 검증이 작동한다", async () => {
      // 상품 수정 버튼 클릭
      const editButton = screen.getAllByText("수정")[0];
      fireEvent.click(editButton);

      // 가격 입력 필드에 빈 값 입력 후 blur
      const priceInputs = screen.getAllByPlaceholderText("숫자만 입력");
      const priceInput = priceInputs[0]; // 가격 입력 필드는 첫 번째

      // 빈 값으로 만든 후 blur 이벤트 발생
      fireEvent.change(priceInput, { target: { value: "" } });
      fireEvent.blur(priceInput);

      // onBlur에서 빈 값을 0으로 처리하는 로직이 있음을 확인
      // (실제 DOM value는 React의 상태 관리에 따라 다를 수 있음)
      await waitFor(() => {
        // 빈 값이거나 0이면 정상
        expect(priceInput.value === "" || priceInput.value === "0").toBe(true);
      });
    });

    test("상품 재고 입력 시 빈 값 검증이 작동한다", async () => {
      // 상품 수정 버튼 클릭
      const editButton = screen.getAllByText("수정")[0];
      fireEvent.click(editButton);

      // 재고 입력 필드에서 빈 값으로 만든 후 blur
      const stockInputs = screen.getAllByPlaceholderText("숫자만 입력");
      const stockInput = stockInputs[1]; // 재고 입력 필드는 두 번째
      ...
      await waitFor(() => {
        // 빈 값이거나 0이면 정상
        expect(stockInput.value === "" || stockInput.value === "0").toBe(true);
      });
    });

    test("상품 할인율 입력 시 100 초과 검증이 작동한다", async () => {
      // 상품 수정 버튼 클릭
      const editButton = screen.getAllByText("수정")[0];
      fireEvent.click(editButton);

      // 할인율 입력 필드 찾기 (이미 할인 정책이 있는 상품이므로 첫 번째 할인율 필드 사용)
      const discountRateInputs = screen.getAllByPlaceholderText("%");
      const discountRateInput = discountRateInputs[0]; // 첫 번째 할인율 필드

      // 150% 입력 (100 초과)
      fireEvent.change(discountRateInput, { target: { value: "150" } });
      fireEvent.blur(discountRateInput);
      ...
      // 값이 100으로 수정되었는지 확인
      expect(discountRateInput.value).toBe("100");
    });

    test("상품 할인율 입력 시 음수 검증이 작동한다", async () => {
      // 상품 수정 버튼 클릭
      const editButton = screen.getAllByText("수정")[0];
      fireEvent.click(editButton);

      // 할인율 입력 필드 찾기 (두 번째 할인율 필드 사용)
      const discountRateInputs = screen.getAllByPlaceholderText("%");
      const discountRateInput = discountRateInputs[1]; // 두 번째 할인율 필드
      ...
      await waitFor(() => {
        expect(
          screen.getByText("할인율은 0% 미만일 수 없습니다")
        ).toBeInTheDocument();
      });

      // 값이 0으로 수정되었는지 확인
      expect(discountRateInput.value).toBe("0");
    });
