...
function App() {
  ...
  const { products, addProduct, updateProduct, removeProduct, setSearchKeyword } = useProducts();
  const { coupons, addCoupon, removeCoupon } = useCoupons();
  const { cart, addToCart, removeFromCart, updateQuantity, applyCoupon, selectedCoupon, cartTotal, clearCart, totalQuantity } = useCart(showToast);
        ...
        {page === 'shop' ? (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <div className="lg:col-span-3">
              <ProductList products={products} onAddToCart={addToCart} onSearch={setSearchKeyword} />
            </div>
